...
pub const EASY_VALUE_TYPE_INT64: EasyValueType = 10;
...
impl EasyCodec {
    ...
    pub fn add_i64(&mut self, key: &str, val: i64) {
        let v = val.to_le_bytes().to_vec();
        self.add_val(EASY_KEY_TYPE_USER, key, EASY_VALUE_TYPE_INT64, v);
    }
    ...
    pub fn to_json(&self) -> String {
        ...
        for i in self.items.iter() {
            ...
            } else if i.value_type == EASY_VALUE_TYPE_INT64 {
                let val = parse_le_i64(i.value.as_slice(), 0);
                str_json.push_str(val.to_string().as_str());
            } else if i.value_type == EASY_VALUE_TYPE_BYTES {
                // let val = encode(i.value.as_slice());
                let val = general_purpose::STANDARD.encode(i.value.as_slice());
                str_json.push_str("\"");
...
impl EasyCodecItem {
    ...
    pub fn get_int64(&self) -> Result<i64, String> {
        ...
        Err("value type not i64".to_string())
    }
...
fn parse_le_i64(slice: &[u8], index: usize) -> i64 {
    i64::from_be_bytes([
        slice[index + 0],
        slice[index + 1],
        slice[index + 2],
...
mod tests {
    ...
    fn easy_codec_test() {
        ...
        let aa: i64 = 456;
        let item11 = EasyCodecItem {
            key_type: EASY_KEY_TYPE_USER,
            key: "key11".to_string(),
            value_type: EASY_VALUE_TYPE_INT64,
            value: aa.to_le_bytes().to_vec(),
        };
        ...
        let items = vec![item1, item11, item2, item3];
        let ec = EasyCodec::new_with(items);
        ...
        let key2 = ec.get_i64("key2").unwrap();
